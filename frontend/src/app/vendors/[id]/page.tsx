...
export default function VendorDetailPage() {
  ...
  const { data: vendor } = useQuery({
    ...
    queryFn: () => vendorsClient.get(vendorId),
  });

  const { data: contacts = [] } = useQuery({
    queryKey: ["vendorContacts", vendorId],
    queryFn: () => vendorsClient.listContacts(vendorId),
  });

  const addContactMutation = useMutation({
    mutationFn: () => vendorsClient.createContact(vendorId, {
  ...
  const deleteContactMutation = useMutation({
    mutationFn: (contactId: string) => vendorsClient.deleteContact(vendorId, contactId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["vendorContacts", vendorId] });
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent className="space-y-3">
            {vendor.email && <div className="flex items-center gap-2"><Mail className="h-4 w-4 text-muted-foreground" />{vendor.email}</div>}
            {vendor.phone && <div className="flex items-center gap-2"><Phone className="h-4 w-4 text-muted-foreground" />{vendor.phone}</div>}
            {vendor.website && <div><span className="text-muted-foreground text-sm">Website:</span> {vendor.website}</div>}
            {vendor.notes && <div><span className="text-muted-foreground text-sm">Notes:</span><p className="mt-1">{vendor.notes}</p></div>}
          </CardContent>
        </Card>
              ...
              <Dialog open={addContactOpen} onOpenChange={setAddContactOpen}>
                ...
                <DialogContent>
                  ...
                  <div className="space-y-4">
                    ...
                    <div><Label>Role</Label><Input value={contactRole} onChange={(e) => setContactRole(e.target.value)} /></div>
